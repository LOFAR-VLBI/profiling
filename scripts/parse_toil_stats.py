...
def parse_stats(file):
    ...
    for name, job in data["job_types"].items():
        ...
        if job["max_cores"] > max_cores:
            ...
            max_cores_name = name
        if job["max_time"] > max_walltime:
            max_walltime = job["max_time"]
            max_walltime_name = name
    ...
    print(
        f"{(data['jobs']['total_clock']/3600 + data['jobs']['total_wait']/3600):.3f} h used+wait"
    )
    print(f"Max RAM of {max_ram/1e6:.3f}GB used by {max_ram_name}")
    print(f"Max CPU hours of {max_cpu/3600:.3f}h used by {max_cpu_name}")
    print(
        f"Max CPU hours of {max_cpu_wait/3600:.3f}h spent waiting by {max_cpu_wait_name}"
    )
    print(
        f"Max CPU hours of {max_cpu_total/3600:.3f}h (used+wait) by {max_cpu_total_name}"
    )
    print(f"Max cores used {max_cores} by {max_cores_name}")
    print(f"Longest running job was {max_walltime_name} with {max_walltime/3600:.3f}h")

    if os.path.basename(file).startswith("toil_stats"):
        with open(file.replace("json", "csv"), "w") as f:
            f.write("step,cpuhour,cpuwait,cpuused\n")
            f.write(
                f"{file.lstrip("toil_stats").split("_")[:-2]},{(data['jobs']['total_clock']/3600 + data['jobs']['total_wait']/3600):.3f},{data['jobs']['total_wait']/3600:.3f}"
            )
